...
**Authentication & RBAC**
- **Headers supported:**
	- **Authorization:** Bearer <token> — preferred. Provide a token generated by the CLI `ops token generate` command.
	- **X-Actor:** <username> — for local/testing only; bypasses token lookup and maps directly to the named user.
- **Role levels:**
