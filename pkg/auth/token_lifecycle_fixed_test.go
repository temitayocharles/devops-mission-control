...
// TestTokenLifecycleFixed duplicates the original lifecycle test but uses a
// distinct name so it can run even if the original test file is temporarily
// malformed. Run with `-run TestTokenLifecycleFixed`.
func TestTokenLifecycleFixed(t *testing.T) {
    dir := t.TempDir()
    cwd, _ := os.Getwd()
